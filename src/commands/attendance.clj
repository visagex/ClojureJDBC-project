...
(defmethod handle-command [:select :attendance] [{:keys [args]}]
  (try
    (let [query (if (empty? (rest args))
                  (sql/format (-> (h/select :*)
                                  (h/from :ATTENDANCE)))
                  (if (< (Integer/parseInt (first (rest args))) 100)
...
(defmethod handle-command [:help :attendance] [{:keys [args]}]
  (println "FOR USE: ")
  (println "insert attendance : user-id, event-id, rsvp (date)")
  (println "select attendance : blank or user-id or event-id")
  (println "delete attendance : user-id, event-id")
  (println "update attendance : user-id, event-id, name of column you wish to update, new value"))